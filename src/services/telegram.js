  ...
  bot.onText(/\/start(?: (.+))?/, async (msg, match) => {
    ...
    if (userId) {
      try {
        ...
        bot.sendMessage(
          chatId,
          `✅ Ваш акаунт Products успішно прив'язаний! Тепер ви можете використовувати функціонал бота`,
        );
      } catch (error) {
      ...
      bot.sendMessage(
        chatId,
        `Вітаємо в Products! Щоб прив'язати цей Telegram-акаунт до вашого профілю на сайті, будь ласка, перейдіть в особистий кабінет і натисніть "Прив'язати Telegram".\n\nДля допомоги введіть /help`,
      );
    }
  });
  bot.onText(/\/help/, (msg) => {
...
export const sendPasswordResetCode = async (chatId, code) => {
  if (!bot) return;
  const message = `Ваш одноразовий код для скидання пароля в Products: \n\n*${code}* \n\nНікому не повідомляйте цей код`;
